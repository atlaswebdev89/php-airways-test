...
class AirWays
{
    ...
    protected function getWays(array $data, array $query): array
    {
        ...
        $positive = array_filter($data, function ($number) use ($query) {
            if ($number['from'] == $query['to']
                && strtotime($number['depart'])
                > strtotime($query['arrival'])) {
                return true;
            }
    ...
    {
        return abs(strtotime($array[0]['depart'])
            - strtotime($array[count($array) - 1]['arrival']));
    }
    ...
    public function viewCli(array $array): void
    {
        ...
        foreach ($array as $key => $value) {
            $key++;
            echo $key.")".$value['from']." -> ".$value['to']." "
                .$value['depart']." ".$value['arrival'].PHP_EOL;
        }
        echo "Итого: c ".$array[0]['depart']." по ".$array[0]['arrival']
            .PHP_EOL;
    }
}
