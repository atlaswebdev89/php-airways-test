...
try {
    ...
    function getWays(array $data, array $query): array
    {
        ...
        $positive = array_filter($data, function ($number) use ($query) {
            if ($number['from'] == $query['to']
                && strtotime($number['depart']) > strtotime($query['arrival'])
            ) {
                return true;
            } else {
                return false;
    ...
    {
        return abs(
            strtotime($array[0]['depart']) - strtotime(
                $array[count($array) - 1]['arrival']
            )
        );
    }
    ...
    echo "По указанному набору рейсов самый продолжительный маршрут будет такой:"
        .PHP_EOL;
    foreach ($result[$keyLong] as $key => $value) {
        $key++;
        echo $key.")".$value['from']." -> ".$value['to']." ".$value['depart']
            ." ".$value['arrival'].PHP_EOL;
    }
    echo "Итого: c ".$result[$keyLong][0]['depart']." по "
        .$result[$keyLong][0]['arrival'].PHP_EOL;
} catch (Throwable $e) {
